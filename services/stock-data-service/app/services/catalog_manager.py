    ...
    async def get_catalog(self) -> Optional[DataCatalog]:
        ...
        try:
            ...
            if catalog_dict:
                ...
                catalog_dict["last_updated"] = datetime.fromisoformat(catalog_dict["last_updated"])
                
                # Convert symbol dates
                for symbol in catalog_dict.get("symbols", []):
                    symbol["start_date"] = datetime.fromisoformat(symbol["start_date"]).date()
                    symbol["end_date"] = datetime.fromisoformat(symbol["end_date"]).date()
                    symbol["last_updated"] = datetime.fromisoformat(symbol["last_updated"])
        ...
        try:
            ...
            if not daily_exists:
                ...
                symbol_summary = SymbolSummary(
                    symbol=symbol,
                    start_date=datetime.fromisoformat(data_dict["data_range"]["start"]).date(),
                    end_date=datetime.fromisoformat(data_dict["data_range"]["end"]).date(),
