...
class WeeklyDataSyncer:
    ...
    async def sync_symbol(self, symbol: str, force: bool = False) -> dict:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Error syncing {symbol}: {str(e)}"
            }
...
async def sync_weekly_data_for_symbol(
    symbol: str,
    force: bool = Query(False, description="Force regeneration even if weekly data exists")
):
    """
...
async def get_sync_status():
    ...
    return APIResponse(
        ...
        data={
            ...
            "sync_percentage": round(len(weekly_symbols) / len(daily_symbols) * 100, 2) if daily_symbols else 0
        }
    )