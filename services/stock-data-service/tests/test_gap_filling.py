...
def check_for_gaps(data_points: list, expected_start: str, expected_end: str):
    ...
    for date in all_dates:
        if date not in actual_dates.values:
...
async def test_gap_filling():
    ...
    data_2022 = await get_data(symbol, "2022-01-01", "2022-12-31")
    
    # Filter data to 2022 only
    if data_2022.get('data_points'):
        # Filter data points to 2022
        data_points_2022 = [
            dp for dp in data_2022['data_points'] 
        ...
        print("   No data retrieved for 2022")
    ...
    for start, end in ranges_to_check:
        data = await get_data(symbol, start, end)
...
async def test_gap_detection():
    ...
    data_q1 = await get_data(symbol, "2023-01-01", "2023-03-31")
    
    if data_q1.get('data_points'):
        gaps = check_for_gaps(data_q1['data_points'], "2023-01-01", "2023-03-31")
        print(f"   Found {len(gaps)} missing dates")
        ...
        data_q1_filled = await get_data(symbol, "2023-01-01", "2023-03-31")
        if data_q1_filled.get('data_points'):
            gaps_after = check_for_gaps(data_q1_filled['data_points'], "2023-01-01", "2023-03-31")
